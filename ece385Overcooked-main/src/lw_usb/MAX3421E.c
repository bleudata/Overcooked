...
#define BUFFER_SIZE 24 // needed to be greater than 12, 12 got USB ERROR
...
//	xil_printf ("Start reg_wr\n");

	//psuedocode:
	//select MAX3421E
	Status = XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
	if(Status != XST_SUCCESS) {
		xil_printf ("Error in reg_red slave select %d \n", Status);
	}

	// clean out buffers before transfer
	...
	if(Status != XST_SUCCESS) {
		xil_printf ("Error in reg_red slave deselect %d \n", Status);
	}
//	xil_printf ("End reg_wr\n");
}
...
//	xil_printf ("Start bytes_wr number: %d \n", nbytes);
...
//	xil_printf ("End bytes_wr \n");
	return (data + nbytes);
}

/* Single host register read        */
BYTE MAXreg_rd(BYTE reg) {
//	xil_printf ("Start reg_rd\n");
	BYTE val;
	//psuedocode:
	//select MAX3421E (
	Status = XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
	...
	//if return code != 0 print an error
	if(Status != XST_SUCCESS) {
		xil_printf ("Error in reg_rd write reg %d val %d \n", WriteBuffer[0], ReadBuffer[2]);
	}
...
//	xil_printf ("Start byte_rd number: %d\n", nbytes);

	//psuedocode:
	//select MAX3421E (may not be necessary if you are using SPI peripheral)
	Status = XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);
	if(Status != XST_SUCCESS) {
		xil_printf ("Error in  bytes_rd  slave select %d \n", Status);
	}
	...
	WriteBuffer[0] = reg;
	Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, nbytes+1);
	memcpy(data, &ReadBuffer[1], nbytes);

	//read return code from SPI peripheral
	//if return code != 0 print an error
	if(Status != XST_SUCCESS) {
		xil_printf ("Error in bytes_rd  write reg %d \n", Status);
	}
